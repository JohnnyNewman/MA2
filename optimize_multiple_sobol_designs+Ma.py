...
def run(options):
    ...
    soboleng = SobolEngine(n_dv+2, True)
    ...
    AOA_mat = X_draw[0,:] * (alpha_upper-alpha_lower) + alpha_lower
    ...
    for i, dvs in enumerate(dv_list):
        ...
        config.DV_VALUE_OLD = dvs
        dsn = SU2.eval.design.Design(config, folder='DESIGNS/DSN_{:04d}'.format(i))
        inputs.append({"design": dsn, "dvs": dvs, "its": 20, "accu": 1.e-10, "bound_upper": 0.5*scale, "bound_lower": -0.5*scale})
     
    with Pool(processes=7) as pool:
        #outputs = pool.map(run_one_design, inputs)
        outputs = pool.map(run_design_optimization, inputs)
...
def run_design_optimization(inputs, its, accu, bound_upper, bound_lower):
    ...
    its = inputs["its"]
    accu = inputs["accu"]
    bound_upper = inputs["bound_upper"]
    bound_lower = inputs["bound_lower"]
    ...
    lb = np.ones(no_dvs)*bound_lower
    ub = np.ones(no_dvs)*bound_lower
    bounds = Bounds(lb=lb, ub=ub, keep_feasible=True)
    
    nlc_cieq = NonlinearConstraint(design.con_cieq, 0., np.inf, jac=design.con_dcieq, keep_feasible=True)
    nlc_ceq = NonlinearConstraint(design.con_ceq, 0., 0., jac=design.con_dieq, keep_feasible=True)
    
    solver_options = {'gtol': 1e-05, 'norm': inf, 'maxiter': its, 'disp': True, 'return_all': False, 'finite_diff_rel_step': None})
    minimize(fun=design.obj_f, x0=dvs, method="BFGS", jac=design.obj_df, bounds=(), constraints=(nlc_cieq, nlc_ceq), tol=accu, options=solver_options)
